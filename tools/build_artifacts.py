...
UNITS = [
    ...
    dict(key="dspic33ak128mc102", kind="std",
         ...
         #
         # SEPARATE xc-dsc-nm DEFECT (found auditing this file's own output -
         # see platform.md "known toolchain defects"): `xc-dsc-nm --print-size`
         # only populates the size field for symbols whose .size directive
         # survived assembly - about 10% of this port's functions (~50-60 of
         # ~210). Combined with --size-sort, nm silently DROPS every symbol it
         # can't size instead of listing it with a 0/blank size - so the old
         # committed .syms (nm output verbatim) summed to ~8.2KB against a
         # 41816B RELEASE binary, missing `main`, `protocol_main_loop`,
         # `st_prep_buffer`, `gc_execute_line`, and most other functions
         # entirely. Fixed by gen_symbol_map()'s objdump fallback below: for
         # every function nm can't size, `xc-dsc-objdump -t` gives the address
         # and `-h` gives the containing code-section's bounds, so the size is
         # recovered as the delta to the next function's address (capped at
         # the section end for the last function in each chunk). Entries
         # produced this way are marked "(addr-delta estimate)" in the .syms
         # file - they are not to-the-byte exact (padding/alignment/jump-
         # tables between functions land on whichever neighbour is scanned
         # first) but are within a few bytes and, critically, are no longer
         # silently absent.
