...
- **Remaining before "ready for hardware"** (tracked as PLAN.md Phase 3, "SAMD21 Closure"):
  - ⏳ `SysTick_Config()` exists (`core_cm0plus.h`) but is **never called** from startup — `hal_millis()`/`hal_micros()` stay frozen at 0 unless something else drives them. `_delay_ms()`'s busy-wait fallback keeps things functional in the meantime, but this is a real, unfixed gap.
