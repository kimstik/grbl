...
void Reset_Handler(void) {
  ...
  //
  // hal_system_init() -> hal_clock_config() (HSE 25MHz -> PLL 96MHz, flash
  // latency + prefetch raised before the switch) + hal_gpio_init() (RCC
  // AHB1 port clocks, MODER/PUPDR, AF mux) was written, reviewed and
  ...
  // of the three symbols and the chip ran on the 16MHz HSI reset default
  // with unconfigured GPIO. Same class as BUG #21 (LTO deleted the vector
  ...
  //                       latency BEFORE selecting the PLL; stm32_timing_
  //                       init() then derives DWT/SysTick from the final
  //                       frequency.
  //   GPIO after clock  - the RCC AHB1ENR port-clock enables it writes are
