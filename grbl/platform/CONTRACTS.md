...
## 38. Gaps found porting SG2002 (first companion-core port: no flash, no clock to configure, a second cache domain, and an OS on the other side)
...
## 39. A ratchet invoked only from CI is a ratchet the developer never sees - and "boot integrity" has more than one correct shape per ISA
...
## §NEW. The extension axis (`EXT=`): where the edge is, as a separate question from which silicon
...
## §NEW. The segment-runtime seam: `GRBL_SEG_PUBLISH`, `GRBL_STEPPER_TU_EXPORTS`, and why the tuple guard is structural
