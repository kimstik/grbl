...
## 27. Defined but never called + LTO = silently absent; byte-invariance is a WEAK signal (placeholder number — integrator assigns the final one; cite this slug, not "§27", from elsewhere)
...
## 28. A guard in the wrong TRANSLATION PHASE certifies just as falsely as one checking the wrong symbol family — §19's lesson 1, in a new shape (placeholder number — integrator assigns the final one; cite this slug, not "§28", from elsewhere)
