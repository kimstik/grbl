...
// platform.c
extern GRBL_BOOT_INIT void SystemClock_Config(void);

// ============================================================================
// RESET HANDLER (C portion - reached from _start with sp/gp already valid)
//
// __attribute__((used)), NOT GRBL_BOOT_INIT: its only caller is `jal
// Reset_Handler` inside _start's raw inline asm below, invisible to LTO's
// IPA (CONTRACTS.md gap-log, same fix as ch32v006/ch570's startup.c). noinline
// alone would not stop a whole-program-optimizing linker from deciding
// nothing reaches it; `used` forces emission regardless. This is the one
// deliberate exception to boot_init.h's "used would defeat the check" rule -
// it applies to functions whose reachability the check itself is testing,
// not to a symbol reached only through non-C-visible control flow.
// ============================================================================
__attribute__((used))
